...
export default function App() {
  ...
  return (
    <div className="w-screen h-[100dvh] bg-[#2c3e50] font-sans overflow-hidden flex flex-col">
      ...
      {/* 이름 한 번 묻기. ★**지도는 계속 쓸 수 있다** — 위쪽에 뜨는 작은 카드일 뿐이라
          ...
          ★★**「직원등록된 이름만 접속 가능합니다」는 뺐다** (2026-09-05 사용자 지시).
           ...
           그대로 써도 된다. 직원 명단은 있지만 **문지기가 아니라 오너의 눈**이다
           (집계창이 「명단 밖」으로 표시할 뿐 아무도 막지 않는다).
           ★**다시 붙이지 말 것.** 들통난 안내문은 다음 안내문까지 같이 못 믿게 만든다 —
      ...
      {/* 뷰어에서 배를 탭하면 뜨는 카드 — 호선번호·3중점검 위치에 더해, 그 호선의
          ...
          여기에 사이드바 `할일` 탭의 **`준비`** 를 D-day 순으로 같이 보여준다.
          ★★**업무(`work_tasks`)는 올리지 않는다 — 아예 읽지도 않는다**
           (2026-09-06 사용자 지시: 「다른직원들이 보면 안되는건데 … 그쪽은 아예 들여다
           보지도 않게 해야해」). 이 카드는 **뷰어 누구나** 보므로 사내 업무 메모가
           실리면 안 된다. 자세한 경위는 `src/lib/vessel-plan.ts` 머리 주석. */}
      {shipCardOpen && (
        <div
          ref={cardDrag.ref}
