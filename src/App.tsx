  ...
  return (
    <div className="w-screen h-[100dvh] bg-[#2c3e50] font-sans overflow-hidden flex flex-col">
      ...
      {/* 이름 한 번 묻기. ★**지도는 계속 쓸 수 있다** — 위쪽에 뜨는 작은 카드일 뿐이라
          ...
          ★★**글귀와 실제 동작이 다르다** (2026-09-05 사용자가 문구를 직접 지정).
           「직원등록된 이름만 접속 가능합니다」라고 적혀 있지만 **막는 코드는 없다** —
           직원 명부 자체가 없고(사내 SSO 없음), 무엇을 적든 통과하며, 카드를 무시하고
           지도를 그대로 써도 된다. 이름을 적게 하려는 압박 문구다.
           **다음 세션이 "검사 로직이 어디 있지" 하고 찾지 않도록 여기 적어 둔다 — 없다.**
           진짜로 막으려면 명부와 인증이 먼저다(그때 이 주석부터 지운다). */}
      {askName && (
